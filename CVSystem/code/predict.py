...
"""
This code is a Python script that predicts which fingers are touching certain points on a surface, given the coordinates of the fingers and the points of interest.
The script reads input data from CSV files, processes it using pandas and numpy libraries, and writes the output to a new CSV file.
"""
...
def main(z_filename):
    ...
    coord_file = os.path.join(current_directory, 'output_logs', 'test_acc.csv')
    no_z_file = os.path.join(current_directory, 'output_logs', z_filename)
    ...
    destination_path = os.path.join(current_directory, 'output_logs', 'predicted_touching_fingers.csv')
    coord_df.to_csv(destination_path, index=False)
